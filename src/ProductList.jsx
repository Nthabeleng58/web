...
const ProductList = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:9000/products/${updatedProduct.id}`, {
        method: 'PUT',
  ...
  const addStock = async (id) => {
    ...
    if (!isNaN(quantityToAdd) && quantityToAdd > 0) {
        try {
            ...
            const response = await fetch(`http://localhost:9000/products/${id}/stock/add`, {
                method: 'POST',
                headers: {
...
const deductStock = async (id) => {
    ...
    if (!isNaN(quantityToDeduct) && quantityToDeduct > 0) {
        try {
            ...
            const response = await fetch(`http://localhost:9000/products/${id}/stock/deduct`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
